...
process.stdin.on('end', () => {
  try {
    ...
    const devPatterns =
      /\b(npm\s+run\s+(dev|start|serve)|pnpm\s+(dev|start|serve)|nx\s+serve)\b/;
    const isTmux = /^\s*tmux\s+/.test(cmd);

    if (devPatterns.test(cmd) && !isTmux && !process.env.TMUX) {
      console.error(
        '[MODA] BLOCKED: Dev server must run in tmux to avoid hanging the agent',
      );
      console.error(
        '[MODA] Use: tmux new-session -d -s dev "nx serve canvas-fe"',
      );
      process.exit(2);
    }
  } catch {}
